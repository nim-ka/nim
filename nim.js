...
function Nim (functions, variables, conditions) {
	this.functions = functions || default_functions;
	this.variables = variables || {};
	this.outerScope = null;
}

Nim.prototype.tokenize = function (code) {
	let tokens = [];
	...
	for (let index = 0; index < code.length;) {
		...
		switch (true) {
			case char == "#": {
				...
				} else if (["if", "elseif", "else", "def"].indexOf(val) > -1) {
...
Nim.prototype.parse = function (tokens) {
	let result;
	let output = "";
...
Nim.prototype.createChildScope = function () {
	let scope = new Nim();
	scope.outerScope = this;
	return scope;
};

// Returns { result: result, output: output }
Nim.prototype.parseBlock = function (token) {
...
Nim.prototype.shallowParseDataBlockVariable = function (token, output) {
...
Nim.prototype.parseDataOperation = function (data, data2, operation) {
	switch (operation) {
		case "+": {
			data += data2;
			break;
...
// Returns { result: result, output: output }
Nim.prototype.parseStatement = function (statement) {
	let result;
	let output = "";
	...
	switch (token.type) {
		...
		case "variable": {
			...
			if (!this.conditions.length) {
				throw new Error("elseif without preceding (else)if");
			}
			...
			if (!this.conditions.reduce((a, b) => a || b) && condition) {
				if (["data", "block", "variable"].indexOf(token.type) < 0) {
					throw new Error(`Expected code body, got token of type ${token.type}, value ${token.value}`);
				}
			...
			if (!this.conditions.length) {
				throw new Error("else without preceding (else)if");
			}
...
Nim.prototype.run = function (code) {
